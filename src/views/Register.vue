<template>
  <Navbar />
  <div class="login-background flex items-center justify-center min-h-screen relative overflow-hidden px-4 py-4 md:py-8 pt-16 md:pt-28" 
    ...
    <div class="login-content bg-[#1f1f1f]/95 p-4 sm:p-8 rounded-2xl shadow-xl w-full max-w-md mx-auto text-white">
      ...
      <h1 class="text-xl sm:text-2xl font-bold text-center mb-3 sm:mb-4 text-blue-400">Crear Cuenta</h1>

      <p class="text-center text-gray-400 mb-4 sm:mb-6 text-xs sm:text-sm">
      ...
      <form @submit.prevent="register" class="space-y-3 sm:space-y-4">
        <!-- Nombre completo -->
        <div>
          <label class="block text-xs sm:text-sm font-semibold mb-1.5 sm:mb-2 text-gray-300">Nombre completo</label>
          <div class="relative">
            <input
              v-model="form.fullName"
              type="text"
              placeholder="Tu nombre"
              class="w-full bg-[#2a2a2a] border border-gray-600 rounded-lg px-10 py-2.5 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all"
            />
        ...
        <!-- Correo -->
        <div>
          <label class="block text-xs sm:text-sm font-semibold mb-1.5 sm:mb-2 text-gray-300">Correo</label>
          <div class="relative">
            <input
              v-model="form.email"
              type="email"
        ...
        <!-- Teléfono -->
        <div>
          <label class="block text-xs sm:text-sm font-semibold mb-1.5 sm:mb-2 text-gray-300">Teléfono</label>
          <div class="relative">
            <input
              v-model="form.phone"
              type="tel"
              placeholder="+53 58126024"
              class="w-full bg-[#2a2a2a] border border-gray-600 rounded-lg px-10 py-2.5 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all"
            />
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.95.68l1.5 4.49a1 1 0 01-.5 1.21l-2.26 1.13a11.04 11.04 0 005.52 5.52l1.13-2.26a1 1 0 011.21-.5l4.49 1.5a1 1 0 01.68.95V19a2 2 0 01-2 2h-1C9.72 21 3 14.28 3 6V5z" />
            </svg>
          </div>
        </div>
        ...
        <!-- Contraseña -->
        <div>
          <label class="block text-xs sm:text-sm font-semibold mb-1.5 sm:mb-2 text-gray-300">Contraseña</label>
          <div class="relative">
            <input
            ...
            <svg xmlns="http://www.w3.org/2000/svg" @click="showPassword = !showPassword" :class="['h-5 w-5 absolute right-3 top-1/2 -translate-y-1/2 cursor-pointer transition-colors', showPassword ? 'text-blue-400' : 'text-gray-400']" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div>
          <label class="block text-xs sm:text-sm font-semibold mb-1.5 sm:mb-2 text-gray-300">Confirmar contraseña</label>
          <div class="relative">
            <input
        ...
        <div class="flex items-start gap-3 mt-3 sm:mt-4">
          <input
            id="acceptTerms"
        ...
        <button
          type="submit"
          class="w-full bg-blue-500 text-white py-2.5 rounded-lg font-semibold hover:bg-blue-600 transition-all duration-300 transform hover:scale-[1.02] active:scale-[0.98] mt-4 sm:mt-6">
          Crear cuenta
        </button>
      </form>

      <p class="text-center text-xs sm:text-sm mt-3 sm:mt-4 text-gray-400">
        ¿Ya tienes cuenta?
