  ...
  <Modal :isOpen="isOpen" @close="$emit('close')">
    <div class="bg-transparent rounded-2xl p-6 sm:p-8 border border-transparent text-white">
      <div class="text-center mb-6">
        <!-- Animación de éxito: misma estética que recarga completada -->
        ...
        <h2 class="text-2xl font-bold text-green-400">¡Compra completada!</h2>
        <p class="text-gray-300 mt-1">Recoge tu línea presentando tu carnet en la oficina seleccionada.</p>
      </div>
      ...
      <div class="bg-[#0f1213] rounded-xl p-4 border border-transparent">
        <div class="grid grid-cols-[auto_1fr] gap-x-4 gap-y-3 text-sm items-center">
          <div class="text-gray-400">Nombre y apellidos</div>
          <div class="font-medium text-white text-right">{{ details.fullName }}</div>

          <div class="text-gray-400">Carnet</div>
          <div class="font-medium text-white text-right">{{ details.idCard }}</div>

          <div class="text-gray-400">Oficina de recogida</div>
          <div class="font-medium text-white text-right">{{ details.pickupOffice }}</div>

          <div class="text-gray-400">Código de activación</div>
          <div class="text-right flex items-center justify-end gap-2">
            <code class="bg-black/40 border border-white/10 rounded px-2 py-1 tracking-wider">{{ details.code }}</code>
            <button @click="copyCode" class="p-1.5 rounded hover:bg-white/10 text-gray-300 hover:text-white" title="Copiar código">
...
const props = defineProps<{ isOpen: boolean; details: Details }>()
const emit = defineEmits<{ close: [] }>()

const copyCode = async () => {
  try {
    await navigator.clipboard.writeText(props.details.code)
  } catch {}
}

// Importante: usar toRefs para mantener reactividad al desestructurar props
const { isOpen, details } = toRefs(props)
</script>
